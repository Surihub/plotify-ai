...
def load_data() -> pd.DataFrame:
    ...
    sel_cols = [1, 3, 4, 7, 8, 9, 10, 11, 12, 13]
...
def conclusion_tab() -> None:
    ...
    st.subheader("📝 결론 · 소감")
    st.info(
        "4️⃣ 분석 결과를 정리하고 활동 소감을 작성한 뒤 **한 번에 제출**하세요."
    )
    ...
    # 3) 입력 위젯 --------------------------------------------------------
    concl = st.text_area(
        "🔖 결론 (필수) — 해석과 AI 피드백을 바탕으로 최종 답을 정리하세요.",
        key="conclusion_text",
        height=160,
    )
    ...
    # 4) 전송 버튼 --------------------------------------------------------
    if st.button("📤 결론·소감 전송", key="btn_submit", type="primary", use_container_width=True):
        # ── 입력 검증 ───────────────────────────────────────────
        if not concl.strip() or not feedback.strip():
            st.warning("결론과 소감을 모두 작성해야 전송됩니다.")
            st.stop()
        ...
        # ── (2) 소감 저장 (reflection 시트) ────────────────────
        push_reflection(feedback.strip())

        # ── (3) 완료 알림 ──────────────────────────────────────
        st.success("결론과 소감이 모두 전송되었습니다! 🎉")
        st.balloons()
